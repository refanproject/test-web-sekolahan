...
if (scrollToTopBtn) {
    scrollToTopBtn.addEventListener('click', () => {
        // Adaptive duration based on device and scroll distance
        const startPosition = window.pageYOffset;
        const isMobile = window.innerWidth <= 768;
        const scrollDistance = startPosition;
        
        // Shorter duration for mobile and shorter distances
        let duration;
        if (isMobile) {
            duration = Math.min(600, scrollDistance / 2); // Max 600ms on mobile
        } else {
            duration = Math.min(1000, scrollDistance / 1.5); // Max 1000ms on desktop
        }
